...
- **Video effects** – each tile's ✨ button cycles through Vintage, Washed out, VHS, Frosted, Mirror and Fisheye, then back to off. All but Fisheye use built-in CSS filter functions (or a plain flip) and work on every platform; Fisheye needs an SVG displacement map, the same mechanism Chrome on Android refuses over video
